...
def build_summary(input_path: Path) -> CoverageSummary:
    ...
    statuses = _sample_statuses(coords)
    quality = app_module._build_quality(statuses).model_dump()
    max_batch = app_module.MAX_BATCH
    requires_chunking = len(coords) > max_batch
    ...
    narrative = [
        ...
        (
            f"A real API request would require chunking because {len(coords)} exceeds MAX_BATCH={max_batch}."
            if requires_chunking
            else f"A real API request fits within MAX_BATCH={max_batch}."
        ),
    ]
...
def _format_text(summary: CoverageSummary) -> str:
    ...
    lines = [
        ...
        f"Requires chunking: {summary.requires_chunking} (MAX_BATCH={summary.max_batch})",
        "Narrative:",
    ]
